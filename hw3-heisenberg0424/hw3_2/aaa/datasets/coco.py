...
class CocoCaption(Dataset):
    def __init__(self, root, ann, max_length, limit, transform=train_transform, mode='training'):
        ...
        self.id2image =  {n['id']: n['file_name'] for n in ann['images']}
        ...
        self.tokenizer.enable_padding(direction='right', pad_id =0, pad_Type=0, pad_token='[PAD]', length=self.max_length)
...
def build_dataset(config, mode='training'):
    if mode == 'training':
        ...
        train_dir = os.path.join(config.dir, 'p2_data', 'images', 'train')
        train_file = os.path.join(
            config.dir, 'p2_data', 'train.json')
        ...
        val_dir = os.path.join(config.dir, 'p2_data', 'images', 'val')
        val_file = os.path.join(
            config.dir, 'p2_data', 'val.json')
