...
class MyDataset(Dataset):
    def __init__ (self,path):
        ...
        for i in data:
            i[0] = os.path.join(path,'data',i[0])
...
class DDPM(nn.Module):
    ...
    def mysample(self, n_sample, size, device, guide_w = 0.0, path='',test=False,second=False):
        # we follow the guidance sampling scheme described in 'Classifier-Free Diffusion Guidance'
        # to make the fwd passes efficient, we concat two versions of the dataset,
        # one with context_mask=0 and the other context_mask=1
        # we then mix the outputs with the guidance scale, w
        ...
        for i in tqdm(range(self.n_T, 0, -1)):
            ...
            if (test):
                if (i%80==0 or i== 1):
                    save_image(x_i[0]*0.5+0.5,os.path.join(path,'time'+str(i)+'.png'))

        x_i = x_i * 0.5 + 0.5

        if(test):
            grid = make_grid(x_i, nrow=10)
            save_image(grid,os.path.join(path,'grid.png'))
            return
        ...
        for i in range(n_sample//10):
            if second:
                ...
                label = str(i+1)    
            filename = '_'+label.zfill(3)+'.png'
            for j in range(10):
                save_image(x_i[i*10+j],os.path.join(path,str(j)+filename))
        

def loadModel(pth):
    model = DDPM(nn_model = ContextUnet(in_channels = 3, n_feat = 256, n_classes = 10), betas = (1e-4,0.02), n_T = 400,device = 'cuda', drop_prob = 0.1)
