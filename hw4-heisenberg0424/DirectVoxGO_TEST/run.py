...
def render_viewpoints(model, render_poses, HW, Ks, ndc, render_kwargs,
    ...
    if savedir is not None and dump_images:
        for i in trange(len(rgbs)):
            ...
            only_filename = HW[i]
            if '/' in only_filename:
                only_filename = only_filename.split('/')[-1]
            
            filename = os.path.join(savedir, '{}.png'.format(only_filename))
...
def load_everything(args, cfg, jspath):
    '''Load images / poses / camera settings / data split.
    '''
    data_dict = load_data(cfg.data, jspath)
...
if __name__=='__main__':
    ...
    # load images / poses / camera settings / data split
    data_dict = load_everything(args=args, cfg=cfg, jspath=args.jsonpath)
    ...
    if args.render_test:
        ...
        print('All results are dumped into', args.outputpath)
        rgbs, depths, bgmaps = render_viewpoints(
                render_poses=data_dict['poses'][data_dict['i_test']],
                HW=data_dict['images'],
                Ks=data_dict['Ks'][data_dict['i_test']],
                #gt_imgs=[data_dict['images'][i].cpu().numpy() for i in data_dict['i_test']],
                savedir=args.outputpath, dump_images=1,
                **render_viewpoints_kwargs)
